...
- 文字起こし結果・整形結果（ジャーナル）は、あなたの端末の**保管庫フォルダ**（既定: ドキュメント/QuickScribe）に**プレーンな Markdown / テキストファイル**として保存されます。本アプリの開発者を含む第三者がこれらにアクセスすることはありません。
...
保管庫フォルダ内のファイルを削除すれば、対応するジャーナルは端末から削除されます。APIキーは設定から削除できます（OSセキュアストレージから消去されます）。
