...
- **エントリ（entry）**: 1回の文字起こし／整形結果として保管庫に書き出されるテキストファイル（`note-<yyyymmdd>-<ラベル>.txt`。命名は [ADR-0032](../../adr/0032-content-based-entry-filenames.md)）。
...
Feature: 保管庫へのエントリ永続化
  ...
  Scenario: 同名の衝突で上書きしない (R5)
    ...
    Then "note-20260624-今日のメモ-2.txt" として保存され、既存ファイルは保持される
