...
- **エントリ（entry）**: 1回の文字起こし／整形結果として保管庫に書き出されるテキストファイル（`note-<timestamp>.txt`）。
...
Feature: 保管庫へのエントリ永続化
  ...
  Scenario: 同一タイムスタンプの衝突で上書きしない (R5)
    Given 保管庫に "note-20260624-120000.txt" が既に存在
    When 同じ秒に別エントリを保存する
    Then "note-20260624-120000-2.txt" として保存され、既存ファイルは保持される
