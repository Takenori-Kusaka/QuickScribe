...
- **R4（整合性）達成**: モデル2種＋DLLアーカイブ（sherpa-onnx **v1.12.9** win-x64-shared＝sherpa-rs-sys 0.6.8 とABI一致）の SHA256/サイズをピン留め。アーカイブSHA256はピン留め値と一致確認済み。
- **R5（オンデマンド＋abort安全）達成**: 実配布経路を実機実証。**DLLを exe横に置かない exe が起動**（遅延ロード `/DELAYLOAD:sherpa-onnx-c-api.dll`）→ tar.bz2 を bzip2-rs+tar で **7 DLL 展開**（実装 `extract_dlls` と同一）→ `SetDllDirectory` → **話者分離成功**（DLLは実行時展開先から解決）。取得/検証失敗時は sherpa 未呼び出しでラベル無しへフォールバック＝abort回避。
- **配布**: `tauri.release.conf.json` の bundle.resources は THIRD-PARTY-NOTICES.txt のみ＝ネイティブlibは installer 非同梱。基本インストーラは単一バイナリのまま（ADR-0012）。release.yml windows-x64 は `--features vulkan,diarization`。
- **残（Phase2 継続）**: 日本語実音声での最適閾値・分離精度（ADR-0031 留保）／ CI フルビルド(test-build/release)の完走確認／埋め込み重みライセンス最終確認。
