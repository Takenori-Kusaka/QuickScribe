...
**定常値の幅は 0.08〜0.16 %（絶対幅 0.08 pt / N=5）。** 旧定義の 4.05〜8.78 %（絶対幅 4.73 pt）から**約 59 倍**縮んだ。

第 1 区間（旧定義そのまま）は 6.65 / 7.12 / 20.12 / 8.49 / 10.06 % と依然大きく振れる。**旧 3 本のバラつきの正体が「起動処理の残りをどれだけ掴んだかの当たり外れ」だったことの直接の証拠。**
...
> 定常ピークの実測値はまだ無い（この列を追加したランがこれから走る）。**「153 MB 前後だろう」は推測であり、確定値ではない。**
