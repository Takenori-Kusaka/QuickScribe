...
fn main() {
    ...
    // [QuickScribe patch / ADR-0012] CPU移植性: ggml既定の GGML_NATIVE=ON(=-march=native)は
    // ビルドマシンCPU依存で非決定的(AVX512搭載ランナーでビルドすると非対応CPUの実ユーザーでSIGILL)。
    // GGML_NATIVE=OFF で決定的な AVX2 ベースライン(GGML_AVX/AVX2/FMA/F16C 既定ON, AVX512はOFF)にする。
    // QS_SIMD=avx512 のとき GGML_AVX512=ON(SIMD最適化版ビルド用 / Phase2)。
    config.define("GGML_NATIVE", "OFF");
    if std::env::var("QS_SIMD").as_deref() == Ok("avx512") {
